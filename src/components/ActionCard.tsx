...
// Map of valid Tailwind gradients
...
const ActionCard = ({action, onclick} : { action: QuickActionType; onclick: () => void }) => {
  return (
    <Card className='group relative overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-lg cursor-pointer' onClick={onclick}>
        {/* ACTION GRADIENT */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${GRADIENTS[action.gradient]} opacity-100 group-hover:opacity-50 transition-opacity`}
      />
